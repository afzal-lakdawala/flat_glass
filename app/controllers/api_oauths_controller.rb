class ApiOauthsController < ApplicationController
  ...
  def create
    ...
    a = current_user.api_oauths.ga.where(refresh_token: j["refresh_token"]).first
    if j["refresh_token"].present? and a.blank?
      a = Api::Oauth.new(user_id: current_user.id, app: "GA", refresh_token: j["refresh_token"])
    end
